...
// Edited on 5/13/2020 to show how to check for failed malloc.
...
int main()
{
  ...
  ***/
  ...
  // We succeeded to make the array, so run our binary searches.
  if (numbers != NULL)
  {
    ...
    // Print out the numbers to check.
    for (i = 0; i < 30; i++)
      printf("%d ", numbers[i]);
    printf("\n");
  ...
  {
    printf("Sorry we couldn't allocate that much memory.\n");
  }
