...
/*
gcc -g -O2 -std=c11 -o a wheretosita.c -lm && ./a
...
int perm(Attendee* attendeeArray, int* permutation, int* used,
  ...
  if (index == attendeeCount) {
    // increment permCount by 1 if valid permutation
    return isValid(attendeeArray, attendeeCount, permutation) ? 1 : 0;
  }
  ...
  for (int i = 0; i < attendeeCount; i++) {
    ...
    permutation[index] = i;
    permCount +=
        perm(attendeeArray, permutation, used, attendeeCount, index + 1);
    used[i] = 0;
  }
