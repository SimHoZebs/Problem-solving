...
function sanitized(rawBoard: string) {
  ...
  for (let i = 6; i <= boards.length + 1; i += 6) {
    let board = boards.slice(i - 6, i - 1).map(
      row => row.split(" ").filter(
        cell => cell.length > 0
      )
    );
...
function createCountingBoard(boards: number[][][]) {
  return boards.map(
    board => {
      return ({
...
function calcFinalScore(balls: number[]) {
  if (!result) return;
  const winnerBoard = boards[result.boardIndex];
  const sumOfWinnerBoard = winnerBoard.reduce((prevRow, currRow) => (
    prevRow + currRow.reduce(
      (prev, curr) => (prev + curr)
    )
  ), 0);
