...
function sanitized(rawBoard) {
    let boards;
    boards = rawBoard.split("\n");
    let newBoardArray = [];
    for (let i = 6; i <= boards.length + 1; i += 6) {
        let board = boards.slice(i - 6, i - 1).map(row => row.split(" ").filter(cell => cell.length > 0));
        let newBoard = board.map(row => row.map(cell => parseInt(cell)));
        newBoardArray.push(newBoard);
    }
    return newBoardArray;
}
function createCountingBoard(boards) {
    return boards.map(board => {
        return ({
            row: Array.from({ length: board[0].length }, (v, i) => 0),
            col: Array.from({ length: board[0].length }, (v, i) => 0)
        });
    });
}
;
function findLastWinner(balls) {
    let winnerBoardIndexArray = [];
    ...
    for (let ballIndex = 0; ballIndex < balls.length; ballIndex++) {
        ...
        for (let boardIndex = 0; boardIndex < boards.length; boardIndex++) {
            ...
            for (let rowIndex = 0; rowIndex < boards[boardIndex].length; rowIndex++) {
                ...
                if (colIndex !== -1) {
                    countingBoard[boardIndex].row[rowIndex] += 1;
                    countingBoard[boardIndex].col[colIndex] += 1;
                    break;
                }
            }
        }
...
function calcFinalScore(balls) {
    if (!result)
        return;
    const winnerBoard = boards[result.boardIndex];
    const sumOfWinnerBoard = winnerBoard.reduce((prevRow, currRow) => (prevRow + currRow.reduce((prev, curr) => (prev + curr))), 0);
    let sumOfCalledBalls = 0;
    for (let i = 0; i <= result.ballIndex; i++) {
        for (let k = 0; k < winnerBoard.length; k++) {
            if (winnerBoard[k].includes(balls[i])) {
                sumOfCalledBalls += balls[i];
                continue;
            }
        }
    }
    return (sumOfWinnerBoard - sumOfCalledBalls) * balls[result.ballIndex];
}
