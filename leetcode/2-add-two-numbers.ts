...
function addTwoNumbers(l1: ListNode, l2: ListNode, carry: number = 0): ListNode | null {
...
function makeListNode(arr: number[], index: number = 0) {

  if (index === arr.length - 1) {
    return new ListNode(arr[index]);
  }
  const node: ListNode = new ListNode(arr[index], makeListNode(arr, index + 1));
